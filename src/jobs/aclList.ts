...
export function aclListJob(deploymentName: string, withDemoParams: WithDemoParams): JobDefinition {
  return {
    id: "aclList",
    ...
    run: async (hooks, ctx) => {
      const params = withDemoParams<CreateNetworkACLListParams>("aclList", {
        vpcid: ctx.resources.vpc,
        name: `${deploymentName}-acl`,
      });
      const result = await runAsyncJob("createNetworkACLList", () => callApi<JobIdResponse>("createNetworkACLList", params), hooks);
